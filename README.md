...
- **串联验证（v1.2）**：规则可声明变量提取（`extract: 变量名: 正则（恰好 1 个捕获组）`），命中后从响应取值，后续规则请求文本以 `{{变量名}}` 引用，引擎按依赖自动串联执行——先 GET 提取 id、再 POST 用 `{{id}}` 打命令注入的 jeecg 类漏洞不再需要写死。xray `search`/`set` 与 Nuclei `extractors`（regex / kval / json 纯键路径型）导入时自动映射为该能力（xpath 需人工改写）
...
2. **加 PoC**（首次启动为空库，三选一）：
   - **粘贴导入**：新增 PoC → 粘贴 Xray 或 Nuclei YAML（自动识别格式）→ 转换为 PWF（不支持的字段列进警告，不静默丢弃）→ 保存
   - **批量导入**：新增 PoC → 「从目录批量导入」→ 选目录先预览（可导入/重复/失败逐条列出，确认后才入库）→ 递归 .yaml/.yml，单批上限 2000 文件，单文件失败不中断整批
   - **批量导出**：列表勾选多条 → 导出为合并 YAML（--- 分隔），与单条导出格式一致
   - **本地演示站**：`go run ./cmd/demo-site` 起一个带六类"故意留洞"的靶场（时间盲注/串联提取/信息泄露/正则/TCP banner，另含反例页），用于验证 PoC 真实有效性——GUI 里目标填 `http://127.0.0.1:18080`
